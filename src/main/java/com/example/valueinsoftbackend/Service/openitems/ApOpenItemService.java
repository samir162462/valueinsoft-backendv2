...
@Service
public class ApOpenItemService {
    ...
    public ApOpenItemService(DbApOpenItem repository,
    ...
    public BigDecimal purchaseRemaining(int companyId, int branchId, int supplierId, long sourceId) {
        return repository.getPurchaseRemaining(companyId, branchId, supplierId, sourceId);
    }

    @Transactional
    public long createPurchaseOpenItem(int companyId, int branchId, int supplierId, long stockLedgerId,
                                       BigDecimal remaining, java.time.LocalDateTime receiptTime,
                                       String receiptIdempotencyKey, String actor) {
        if (remaining.signum() <= 0) return 0;
        int terms = repository.getSupplierPaymentTermsDays(companyId, branchId, supplierId);
        return repository.createOpenItem(companyId, branchId, supplierId, "PURCHASE", stockLedgerId,
                "PURCHASE-" + stockLedgerId, receiptTime, receiptTime.plusDays(terms), companyCurrency(companyId),
                remaining, "purchase:" + normalizedKey(receiptIdempotencyKey) + ":" + stockLedgerId, actor);
    }

    @Transactional
    public OpenItemsWriteModels.AllocationResult allocateReceipt(int companyId, int branchId, int supplierId,
                                                                  int receiptId,
        ...
        requireSource(receipt.status(), receipt.branchId(), receipt.partyId(), branchId, supplierId);
        BigDecimal available = receipt.amount().subtract(repository.sumActiveAllocationsForReceipt(companyId, receiptId));
        return allocate(companyId, branchId, supplierId, receiptId, null, receiptId, command,
                available, command.currencyCode(), actor, null);
    }

    @Transactional
    public OpenItemsWriteModels.AllocationResult applyDebitNote(int companyId, int branchId, int supplierId,
                                                                 long debitNoteId,
                                                                 OpenItemsWriteModels.AllocationCommand command,
                                                                 String actor) {
        OpenItemsWriteModels.NoteLock note = repository.findDebitNoteForUpdate(companyId, debitNoteId);
        requireSource(note.status(), note.branchId(), note.partyId(), branchId, supplierId);
        if (!note.currencyCode().equalsIgnoreCase(command.currencyCode())) {
            fail("OPEN_ITEMS_CURRENCY_MISMATCH", "Debit note currency does not match the allocation currency");
        }
        return allocate(companyId, branchId, supplierId, null, debitNoteId, debitNoteId, command,
                note.unappliedAmount(), note.currencyCode(), actor, note);
    }

    private OpenItemsWriteModels.AllocationResult allocate(int companyId, int branchId, int supplierId,
        ...
        String prefix = "ap:" + hash(companyId + ":" + sourceKind + ":" + sourceId + ":"
        ...
        if (available.signum() <= 0) fail("OPEN_ITEMS_SOURCE_EXHAUSTED", "Receipt or debit note has no unallocated amount");

        Map<Long, BigDecimal> requested = requested(command.allocations());
        List<OpenItemsReadModels.OpenItem> locked = requested.isEmpty()
                ? repository.findSettleableForUpdate(companyId, branchId, supplierId, currencyCode)
                : repository.findOpenItemsForUpdate(companyId, requested.keySet().stream().sorted().toList());
        List<Target> targets = buildTargets(locked, requested, available, companyId, branchId, supplierId, currencyCode);
        if (targets.isEmpty()) {
        ...
        for (Target target : targets) {
            // Keep the allocation currency byte-for-byte aligned with the locked open item.
            // This supports legacy currency casing while the service still validates identity
            // case-insensitively at the API boundary.
            String allocationCurrency = target.item.currencyCode();
            long allocationId = repository.insertAllocation(companyId, branchId, supplierId, receiptId, noteId,
                    target.item.openItemId(), target.amount, allocationCurrency, prefix + target.item.openItemId(), actor);
            ...
            inserted.add(new OpenItemsWriteModels.AllocationRow(allocationId, receiptId, noteId,
                    target.item.openItemId(), branchId, supplierId, allocationCurrency, target.amount,
        ...
        if (note != null) {
            ...
            repository.updateDebitNoteApplication(companyId, note.noteId(), applied, unallocated,
    ...
    public OpenItemsWriteModels.AllocationLine reverseAllocation(int companyId, long allocationId,
        ...
        OpenItemsWriteModels.NoteLock note = original.noteId() == null ? null
                : repository.findDebitNoteForUpdate(companyId, original.noteId());
        long reversalId = repository.insertAllocationReversal(companyId, original,
                "ap-reversal:" + hash(allocationId + ":" + normalizedKey(idempotencyKey)), actor);
        ...
        if (note != null) {
            ...
            repository.updateDebitNoteApplication(companyId, note.noteId(), applied, unapplied,
    ...
    public void reverseReceipt(int companyId, int receiptId, String reason, String actor) {
        ...
        // GL reversal (Stage 4.2): supplier payments post as payment/supplier_payment with
        // sourceId "supplier-receipt-<srId>" (FinanceOperationalPostingService.enqueueSupplierPayment).
        // A null result means the receipt never posted to the GL (legacy) — subledger-only is complete.
        financePosting.enqueueSubledgerGlReversal(companyId, receipt.branchId(),
                "payment", "supplier_payment", "supplier-receipt-" + receiptId,
                "supplier-receipt-reversal-" + receiptId, reason, actor);
    }
    ...
    private List<Target> buildTargets(List<OpenItemsReadModels.OpenItem> items, Map<Long, BigDecimal> requested,
        ...
        List<OpenItemsReadModels.OpenItem> ordered = requested.isEmpty() ? items
    ...
    private static OpenItemsWriteModels.AllocationResult result(long sourceId, BigDecimal amount, BigDecimal available,
                                                                 List<OpenItemsWriteModels.AllocationRow> rows, boolean replay) {
    ...
    private static void requireSource(String status, int sourceBranch, int sourceParty, int branchId, int partyId) {
    ...
    private static String normalizedKey(String key) { return key == null || key.isBlank() ? "auto" : key.trim(); }
